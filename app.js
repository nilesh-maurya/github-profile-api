...
app.get("/api/:username", async (req, res, next) => {
  ...
  const userPromise = scrape.getUserData(username);
  const pinnedPromise = scrape.getPinnedRepo(username);
  const repoDataPromise = scrape.getEachRepo(username);

  try {
    const [user, pinnedRepo, repoData] = await Promise.all([
      userPromise,
      pinnedPromise,
      repoDataPromise,
    ]);
...
app.get("/api/:username/repos", async (req, res) => {
  ...
  const repoData = await scrape.getEachRepo(username, after);
